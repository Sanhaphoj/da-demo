// --- Cryptographically Secure Password Hashing (ASVS Compliant) ---

export async function hashPassword(password) {
  // Pure JS SHA-256 Fallback for non-secure contexts (e.g. file:// protocol)
  function sha256Fallback(ascii) {
    ...
    const l = msg.length;
    const paddingLen = (56 - (l + 1) % 64 + 64) % 64;
  ...
  if (typeof window !== 'undefined' && window.crypto && window.crypto.subtle) {
    try {
      ...
      console.warn("Web Crypto API failed, using pure JS SHA-256 fallback:", e);
